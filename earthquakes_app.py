...
if df_new is not None:
    ...
    with col1:
        st.subheader("📍 위치 입력")
        # 사용자로부터 위도, 경도 입력 받기
        lat = st.number_input("위도 입력 (-90.0 ~ 90.0)", min_value=-90.0, max_value=90.0, value=st.session_state.lat, step=0.1)
        lon = st.number_input("경도 입력 (-180.0 ~ 180.0)", min_value=-180.0, max_value=180.0, value=st.session_state.lon, step=0.1)
        
        predict_button = st.button("위험도 예측 및 지도 표시", type="primary")
        
        # 버튼을 누르면 상태를 True로 변경하고 입력값 고정
        if predict_button:
    ...
    with col2:
        ...
        if st.session_state.predicted:
            ...
            folium.Marker(
                ...
                popup=f"입력 위치 ({current_lat}, {current_lon})"
            ).add_to(m)
            
            # 지도 시각화 (Key 값을 주어 렌더링 유지)
            st_folium(m, width=800, height=500, key="earthquake_map")
